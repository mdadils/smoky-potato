...
export const connectElements = (container, svg, path, startElem, endElem) => {
  ...
  const startX = startCoord.left + startCoord.width - svgLeft; // x = left offset + 0.5*width - svg's left offset
  const startY = startCoord.top + startCoord.height * 0.5 - svgTop; // y = top offset + height - svg's top offset

  // calculate path's end (x,y) coords
  //   const endX = endCoord.left + 0.5 * endCoord.width - svgLeft;
  //   const endY = endCoord.top - svgTop;
  const endX = endCoord.left - svgLeft;
  const endY = endCoord.top + endCoord.height * 0.5 - svgTop;
